...
- **Read Committed (RC)**： 每次讀取都會看到資料庫當下「最新且已提交」的數據。這代表如果你讀兩次，中間有人改過並提交，你兩次讀到的結果會不一樣（會發生「不可重複讀」）。
- **Repeatable Read (RR)**： 鎖定交易開始當下的數據快照（Snapshot）。這代表在同一個交易內，無論讀幾次，讀到的結果都一樣，即使中間有人改過並提交，你也看不到。



### 📌 實務場景：MVCC 能預防 Race Condition 嗎？ (如庫存爭搶)

這是一個常見的誤區。**MVCC 是一種「讀取」機制**，它確保的是 Transaction 內部的讀取一致性，但它**無法防止多個 Transaction 同時更新同一筆資料**造成的 Race Condition。

#### 庫存競爭場景 (最後一個商品)

假設商品庫存剩 1，User A 和 User B 同時想買。

1. **MVCC 的角色**：能讓 A 和 B 在不互鎖的情況下，同時讀取到「庫存 = 1」。
2. **Race Condition 的發生**：如果不加額外保護，A 和 B 都會執行 `SET stock = 0` 並成功 Commit，導致超賣。

#### 解決方案：Locking (鎖)

要解決寫入衝突，必須依賴 **Row Lock (行鎖)** 而非 MVCC：

- **方案一：在 WHERE 條件中檢查 (Optimistic)**
  ```sql
  UPDATE inventory SET stock = stock - 1 
  WHERE product_id = X AND stock > 0;
  -- 檢查 API 回傳的 affected_rows，若為 0 表示沒搶到
  ```

- **方案二：SELECT FOR UPDATE (Pessimistic Locking)**
  ```sql
  BEGIN;
    -- 對該行上鎖，其他 Transaction 必須等待
