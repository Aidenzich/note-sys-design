...
接下來，你應該花點時間識別並列出系統的**核心實體**。這有助於定義術語，理解設計的核心資料，並為後續建立基礎。這些是你的 API 將交換的實體，也是你的系統將在資料模型中持久化的實體。在實際面試中，這就像草擬一個項目符號列表並向面試官解釋這是你的初稿一樣簡單。
...
盡量為你的實體選擇好的名稱。雖然大多數問題規模很小，用 `foo` 和 `bar` 代替也能通，但有些面試官會利用這個機會觀察你是否擅長電腦科學中最困難的問題之一（命名）。
...
注意我們使用複數資源名稱（tweets，而不是 tweet）。當前使用者是從請求標頭 (Header) 中的身份驗證 Token 推導出來的，而不是從請求本體或路徑參數中獲取。
...
**保持專注！** 候選人常常過早疊加複雜度，導致永遠無法得出完整的解決方案。**專注於一個相對簡單、能滿足核心功能性需求的設計**，然後在「深入探討」階段再疊加複雜度以滿足非功能性需求。在高階設計階段識別出可以增加複雜度的地方（如快取或訊息佇列）是很自然的，我們鼓勵你口頭提出並做個簡單的筆記，然後繼續前進。

在你繪製設計時，你應該向面試官口述你的思考過程。明確說明資料如何流經系統，以及每個請求會改變什麼狀態（在資料庫、快取、訊息佇列等），從 API 請求開始直到回應結束。當請求到達資料庫或持久層時，是開始記錄每個實體相關欄位的好時機。你可以直接在視覺化的資料庫旁邊做這件事。這有助於將資訊保留在相關組件附近，並隨著設計迭代而容易修改。不必太擔心型別，面試官可以推斷，寫出來只會拖慢你的速度。

> **注意：** 不要浪費時間記錄 Schema 中的每個欄位。例如，面試官知道 User 表有名稱、Email 和密碼雜湊，所以你不需要寫下來。相反，專注於與設計特別相關的欄位。

對於我們簡單的 Twitter 範例，你可以一次一個端點地構建設計。

---

### 6. 深入探討 Deep Dives (~10 分鐘)

敏銳的讀者可能已經注意到，我們簡單的 Twitter 高階設計在獲取使用者動態消息 (Feed) 時效率會低得可憐。沒問題！這正是你在深入探討部分要迭代改進的。現在你有了一個高階設計，你要利用剩下的 10 分鐘左右來**強化你的設計**，透過：
(a) 確保它滿足所有非功能性需求
(b) 解決邊緣情況 (Edge Cases)
(c) 識別並解決問題和瓶頸
(d) 根據面試官的探問改進設計
